...
registerBlockType('vl-ct/page', {
	// Block name. Block names must be string that contains a namespace prefix. Example: my-plugin/my-custom-block.
	title: __('VL Container Tabs Page '), // Block title.
	icon: 'admin-plugins', // Block icon from Dashicons → https://developer.wordpress.org/resource/dashicons/.
	category: 'common', // Block category — Group blocks together based on common traits E.g. common, formatting, layout widgets, embed.
	keywords: [
		__('VL Container Tabs Page '),
		__('Container Tabs Page ')
	],
	attributes: {
		title: {
			type: 'string',
			source: 'meta',
			meta: 'vl_ct_page_per_page',
			default: ''
		},
		title: {
			type: 'string',
			source: 'meta',
			meta: 'vl_ct_page_order',
			default: ''
		},
	},
	...
	edit: function(props) {
		...
		const {attributes: {vl_ct_page_order, vl_ct_page_per_page}, setAttributes} = props;
     ...
     <h2> CPTs </h2>
     <CPTSelect updateAttributes={ updateAttributes } vl_ct_page_order={ vl_ct_page_order } vl_ct_page_per_page={ vl_ct_page_per_page } />
     { (vl_ct_page_order && vl_ct_page_per_page) ? (
       <CPTPreview vl_ct_page_order={ vl_ct_page_order } vl_ct_page_per_page={ vl_ct_page_per_page } />
       ) : (
