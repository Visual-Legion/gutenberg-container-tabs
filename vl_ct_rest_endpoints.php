...
* Add meta
*/
if(!function_exists('vl_ct_add_query_meta')) {
  function vl_ct_add_query_meta($wp_query = "") {
...
class My_Rest_Server extends WP_REST_Controller {
  ...
  public function get_gpb_cpt_examples( WP_REST_Request $request ) {
  	...
  	if ($params && array_key_exists('vl_ct_page_order', $params)) {
  		$order = $params['vl_ct_page_order'];
  	}
  	if ($params && array_key_exists('vl_ct_page_per_page', $params)) {
  		$per_page = $params['vl_ct_page_per_page'];
  	}
	  ...
	  $today = date('c');
	    ...
	    return new WP_Error( 'no_tab', 'Couldn\'t find any Tab examples matching your query', array( 'status' => 404 ) );
	  }

		// return $upcoming_events_querry->posts;
		return vl_ct_add_query_meta($upcoming_events_querry)->posts;
		// return $type;
	}
  ...
  public function get_gpb_cpt_latest( WP_REST_Request $request ) {
    ...
    if ( empty( $upcoming_events_querry ) ) {
      return new WP_Error( 'no_tab', 'Couldn\'t find any Tab examples', array( 'status' => 404 ) );
    }
